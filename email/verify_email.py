...
def mail(spreadsheet, column_name): # For now it is duplicated, but will have different behaviour later
    ...
    for record in spreadsheet.records():
        ...
        #   The part below is a simple template that to either send or skip an email
        #   The plan is to allow the user to then edit the email in some console or text editor
